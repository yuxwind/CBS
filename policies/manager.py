...
class Manager:
    ...
    def __init__(self, args):
        ...
        if  'comb' in args.prune_class or 'woodfisher' in args.prune_class or 'woodtaylor' in args.prune_class or 'grad' in args.prune_class or 'kfac' in args.prune_class:
    ...
    def run(self):
        ...
        for epoch in range(self.initial_epoch, self.n_epochs):
            ...
            if self.args.eval_fast:
                ...
                after_prune_acc_top1, after_prune_loss, \
                        after_prune_acc_top5 =  my_test_dataset(
                                self.args, self.model, 
                                data_test, batched=self.args.batched_test, 
                                device=self.device)
                #if self.args.prune_class == 'comb':
                #    after_prune_acc_top1_train, after_prune_loss_train, \
            ...
            if epoch == 1:
                save_checkpoint(epoch, self.model_config, self.model, self.trainers[0].optimizer,
                                self.trainers[0].lr_scheduler, self.run_dir, is_best=False)
